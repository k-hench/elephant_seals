...
plot_plink_comparison <- \(focal_ind = "ES2551"){
    ...
    data_bcftools <- read_tsv(here(glue("results/roh/bcftools/bed/max_certain/roh_cert_{focal_ind}_on_mirang.bed")),
    ...
    p <- data_het_ind |> 
      ...
      scale_y_continuous(breaks = c(-.2 -.2 * as.numeric(factor(plink_folders, levels = plink_folders)), -.1, 0, 1),
                         labels = c(str_c("plink[",seq_along(plink_folders),"]"), "bcftools", 0, 1)) +
   ...
   ggsave(plot = p, filename = glue("results/img/roh/roh_het_ind_plink/{focal_ind}.png"), width = 10, height = 10)
}

plink_folders <- dir(plink_path, pattern = "d[0-9]*$")
data_plink <- plink_folders |>
  map_dfr(summarize_plink_run) |> 
  mutate(y = -.2 -.2 * as.numeric(factor(folder, levels = plink_folders)))
