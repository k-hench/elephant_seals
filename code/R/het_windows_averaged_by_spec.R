...
windowed_avg_het_pheno |>
  ...
  #               fill = as.character(eo)),
  #           color = "transparent") +
  geom_line(aes(x = gpos - gstart, y = avg_het, group = str_c(seqnames, pheno), color = pheno), linewidth = .25, alpha = .2) +
  geom_point(aes(x = gpos - gstart, y = avg_het, group = seqnames, color = pheno), size = .15, shape = 19, alpha = .2) +
  scale_fill_manual(values = c(`0` = rgb(0,0,0,.1), `1` = rgb(0,0,0,0)), guide = 'none') +
  scale_x_continuous(expand = c(0, 0)) +
  # facet_grid(pheno ~ .) +
  # coord_cartesian(xlim = c(0, max(genome_tib$end_pos)),
...
ggsave(filename = here(glue("results/img/het/win_avg_het_pheno_w{sprintf('%.0fMb', window_width /1e6)}_s{sprintf('%.0fkb', window_step /1e3)}.png")),
       # plot = p,
